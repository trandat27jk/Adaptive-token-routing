...
class Cnn_embedding(nn.Module):
    def __init__(self, config,layer_scale_init=1e-6):
        super().__init__()
        self.dwconv=nn.Conv2d(in_channels=config.cnn_dim,out_channels=config.cnn_dim,kernel_size=7,padding=3,groups=config.cnn_dim)
        self.norm=RMSNorm(config.cnn_dim,eps=1e-6)
        self.act=nn.GELU()
        self.pwconv2=nn.Linear(config.cnn_dim,4*config.cnn_dim)
        self.gamma=(nn.Parameter(layer_scale_init*torch.ones((config.cnn_dim)),requires_grad=True) if layer_scale_init>0 else None)
        self.downsample=nn.Conv2d(in_channels=config.channels,out_channels=config.cnn_dim,kernel_size=4,stride=4)
        self.maxPool=nn.MaxPool2d(kernel_size=config.max_pool_size,stride=config.max_pool_stride)
    def forward(self,x):
        x=self.downsample(x)
        input=x
        x = self.dwconv(x)
